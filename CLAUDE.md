...
0. **战略前提**：每次对话前必须先读 `docs/DIRECTION.md`（商业化路径 / 卖点排序 / E2EE 隐私底线 / AI 暂缓 / 对现有 task 的约束）和 `ROADMAP.md`（阶段性上线计划 + 用户必须解锁的资源与法律手续 + 各阶段架构师可独立驱动的工作）。任何技术决策与这两份文档抵触时，先停下来与用户对齐。
