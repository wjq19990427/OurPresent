...
- **task-26**：收尾「我们 / 我的 / 设置」三视图体验：`farewell` 状态改回前端单一 session_state 来源，不再暴露到 URL；「我的」tab 拆成「等待开放 / 私密记录」两段，并补上绑定成功后的首条记录引导。
- 「我们」tab 空状态与首次解锁时刻同步打磨：未绑定时可直接跳到设置绑定区，第一条 shared 记录在 24 小时内会出现一次温和提示，三处空状态文案统一收敛到克制口吻。

### 冻结期销毁链路 UX

- **task-25**：冻结期链路补齐为完整双端 UX：进入冻结期改为二次确认，冻结后双方顶部都会看到剩余天数 banner，并能通过“发起请求 / 对方同意 / 拒绝 / 发起方撤回”完成撤回冻结的双方握手。
- 冻结期内新增“现在分手”申请：任一方都可以发起，对方同意后才会立即永久销毁共同数据，发起方在对方回应前也可以主动撤回申请。
- 两种销毁路径统一落到同一张“温柔分手”页：现在分手被同意，或冻结期到期自动销毁后，都会先停在告别页，由用户自己点“返回首页”再离开。

### Alpha UX 收尾

- **task-24**：打磨「写记录 → 等解锁 → 解锁瞬间」主旅程：首条记录入口默认展开、申请共享默认 1 个月、伴侣端新增不泄露内容的待解锁预告、24 小时内新解锁标识，并把「我们」tab 评论区内嵌到所属卡片下。
- 共享时间线隐藏写作者完整度提示，移动端用 `我的记录 / 对方的记录` 保留作者归属；评论区按私密 / 待解锁 / 已共享重新收敛可见规则。
