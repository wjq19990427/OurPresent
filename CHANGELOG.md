...
- UI 三栏化：旧「记录舱 / 灵感墙 / 已归档 / 情侣空间 / 账户」5 tab 合并为「🏠 我们 / 📝 我的 / ⚙️ 设置」3 tab，默认落地「我们」
- 修复「情侣空间」单向可见 bug：过滤口径从 `user_id == partner_id` 改为 `couple_id + visibility == "shared"`，自己 shared 给伴侣的记录在双方的「我们」都能看到，自己/伴侣的卡片左右分边渲染
- 卡片状态徽章统一为 4 态：`[草稿] / [仅自己] / [倒计时·还有 N 天] / [已分享]`，UI 上不再出现项目自创术语
- 业务层零改动，全部修改收敛在 `frontend/` + `main.py` + L2 契约
