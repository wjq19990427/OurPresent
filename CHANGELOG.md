...
- **task-25**：冻结期链路补齐为完整双端 UX：进入冻结期与立即销毁都改为二次确认，冻结后双方顶部都会看到剩余天数 banner，并能通过“发起请求 / 对方同意 / 拒绝 / 发起方撤回”完成撤回冻结的双方握手。
- 销毁后不再停留在设置页弹冷提示，而是进入一段简短告别页再回到登录；冻结期相关只读提示也统一收敛为更克制的语气。
