...
**最后更新**：2026-05-20  ·  **当前版本**：**v3.1.0**（已发布）  ·  **阶段**：Phase 2 · Wave 1 完成（M1+M2），M3 配对协议启动中  ·  详见 `docs/PHASE2.md` / `docs/ROADMAP.md`
...
- **Wave 1 全部完成（2026-05-20）**：M1 开发者工具模拟器三项加密测试通过（68 KB 纯 JS bundle，@noble 替换 libsodium.js）；M2 手机微信内置浏览器访问 cloudflared /healthz 返回 200；`docs/E2EE.md` 选型同步更新
...
1. **task-29 · M3 配对协议**（任务卡已入库，可派发给实现工）：依赖 AppID 才能真机验收（两台手机扫码配对）
2. **用户阻塞项**：小程序 AppID（M3 真机验收必须；用户说"不得不用时再申请"，M3 是不得不用的时候）
3. **M3 内含技术债清偿**：X25519 切换 `@noble/curves/x25519`（已写入任务卡）
4. **基础设施状态**：服务器（学长的，不用）· ICP 备案办理中（不阻塞 Phase 2）· 域名等备案下来 · 公司未注册（Phase 4 才需要）
