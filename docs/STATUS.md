...
**最后更新**：2026-05-16  ·  **当前版本**：**v3.1.0**（已发布）  ·  **阶段**：Phase 2 · 本地最小闭环（端到端 E2EE，本地优先）·  详见 `docs/PHASE2.md` / `docs/ROADMAP.md`
...
> **战略转向 2026-05-16**：Phase 2 改为**本地优先**——M0~M6 全部在本地 Docker Compose + 内网穿透真机联调完成，上云只是配置切换。学长服务器不共用，等本地跑通后单独要一台。详见 `docs/PHASE2.md`。

1. **Phase 2 唯一阻塞项**：用户去申请小程序 AppID（接口测试号即可）
2. **架构师并行推进**：M1（小程序 libsodium.js 真机 smoke test）+ M2（本地 FastAPI + PG + Minio + Nginx Docker Compose 骨架）可立即并行启动
3. **基础设施状态**：服务器（学长的，不用）· ICP 备案办理中（不阻塞 Phase 2）· 域名等备案下来 · 公司未注册（Phase 4 才需要）
4. **E2EE 协议已落定**：`docs/E2EE.md` 配对协议（X25519 ECDH + SAS）/ 密钥拓扑 / 双路恢复 / libsodium.js 选型就绪
