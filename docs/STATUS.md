...
- **task-24 合并（codex 实现）· Phase 1 W1**：Alpha 写-锁-解锁主旅程 UX 打磨——首条记录入口默认展开 + 上传字段重排（描述 → 感受 → 原因 → 时间）；申请共享默认改为「1 个月后」并加产品文案；待解锁徽章从「⏳ 倒计时 N 天」改为「等待开放 · 还要 N 天/小时/分钟」；「我们」tab 新增**伴侣端预告区块**（只暴露数量+时间，无内容线索）+ 24h 内**「新近解锁」**标识；评论区从 timeline 末尾全局位置改为**紧邻所属卡片内嵌**，并按 visibility 分段（private 仅作者可见可写 / pending_unlock 隐藏 / shared 双方可见可写）；「我们」tab 隐藏写作者完整度提示，移动端用「我的记录 / 对方的记录」前置标签保留作者归属。零 schema 改动
