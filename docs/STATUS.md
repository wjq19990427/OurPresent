...
**最后更新**：2026-05-17  ·  **当前版本**：**v3.1.0**（已发布）  ·  **阶段**：Phase 2 · M1/M2 代码已合并，等待人工验收  ·  详见 `docs/PHASE2.md` / `docs/ROADMAP.md`
...
- **M1 + M2 合并（2026-05-17）**：task-27（M1 libsodium smoke test 页面 + crypto 分包 + vendored sodium.js）+ task-28（M2 FastAPI 服务端骨架，docker compose up 验收通过）均 fast-forward 合并进 master；待用户在微信开发者工具实测 smoke 页面
...
1. **用户待操作（M1 验收）**：用微信开发者工具打开 `miniprogram/` 目录，导航到 smoke 页面，确认三项加密测试全部 `✓ 通过`；记录耗时并回填 `docs/research/wechat-miniprogram-crypto-verified.md`；确认 crypto 分包体积（sodium.js 原始 1290 KB，需看开发者工具实测值）
2. **用户待操作（M2 验收）**：手机微信内置浏览器访问 cloudflared 临时域名下的 `/healthz`，确认返回 200
3. **两项人工验收完成后**：M1 + M2 正式关闭 → 可启动 M3（配对协议）
4. **用户阻塞项**：小程序 AppID（M1 真机验收 + M3 配对协议必须，用户说"不得不用时再申请"）
