...
- **task-20 任务卡入库**：AI 合成数据 workflow MVP 设计落定，独立 `tools/synth/`，MiniMax-M2.5 驱动，三层语义（角色卡 / 时间线 / 延时表达行为）+ DB 隔离 + 必经 application 层
...
1. **task-20 开工**：实现工接 `docs/tasks/task-20.md`，落地 `tools/synth/` 三层合成流水（task-21 PG 迁移 / task-22 FastAPI 包装在 task-20 产出基线后启动）
2. **Phase 2 周报剩余技术债**：A1 + B7（与远程数据库迁移 task-21 一起做）/ B1 暂搁置 / B2 由 task-20 合成语料替代 / C 系列详见 `docs/phase2_audit.md`
...
5. **deferred 到部署阶段**：`tick()` 独立调度器、`load_db()/save_db()` 行级并发改造、累积 `_migrate_db` 清算
6. **Phase 2 其他 AI 模块**：NVC 润色器 / 安全着陆舱 / 专属两人词典（`docs/AI.md` 第二章）
