...
1. **task-20c 待启动**：persona 改"一份 JSON = 一对情侣"、run_synth 一次只生成一份剧本、结局（在一起 vs 销毁）由 LLM 动态判断写入剧本 frontmatter；离线 fallback 读 persona 的 `expected_outcome`；任务卡 `docs/tasks/task-20c.md`，与 task-21 核心文件零交集可并行
2. **task-21 开工**：实现工接 `docs/tasks/task-21.md`，PG + Docker Compose 落地，task-20 四条遗留搭车清理；以 `tools/synth/scripts/任务20_合成数据剧本.md` 重放结果为 SQLite↔PG 跨 DB 一致性验收基线
3. **Phase 2 周报剩余技术债**：A1 + B7（搭车 task-21 处理）/ B1 暂搁置 / B2 由 task-20 合成语料替代 / C 系列详见 `docs/phase2_audit.md`
...
6. **deferred 到部署阶段（task-21 之后）**：`load_db()/save_db()` 整库改行级、`tick()` 独立调度器、累积 `_migrate_db` 清算
7. **task-22**：FastAPI 包装（前端 Streamlit → HTTP → FastAPI → application）
8. **Phase 2 其他 AI 模块**：NVC 润色器 / 安全着陆舱 / 专属两人词典（`docs/AI.md` 第二章）
