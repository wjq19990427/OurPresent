...
1. **另开 task 删除临时手动按钮**：task-9 引入的「🧪 立即生成周报（测试）」在 cron 验证稳定后需清除（`tab_us.py` 中两处 `# TASK-9 临时按钮` 注释为定位标记）
2. **收敛 `app_reports.md` 重复内容**：Report 模型 / reports_repo 描述同时存在于 `domain_models.md` / `infra_db.md` / `app_reports.md`，下一个接触 application 层的任务顺手清理
