...
**最后更新**：2026-05-20  ·  **当前版本**：**v3.1.0**（已发布）  ·  **阶段**：Phase 2 · M1 开发者工具验收通过，M2 等用户手机验收；Wave 1 接近关闭  ·  详见 `docs/PHASE2.md` / `docs/ROADMAP.md`
...
- **M1 开发者工具验收通过（2026-05-20）**：libsodium.js 确认不可用（WASM/UMD/随机源三层问题），加密栈切换为 `@noble/ciphers` + `@noble/hashes`（纯 JS，68 KB）；开发者工具模拟器三项测试全部通过（X25519 5ms / AEAD 1ms / Argon2id 3361ms）；`docs/E2EE.md` 选型已同步更新
- **M1 + M2 合并（2026-05-17）**：task-27/28 fast-forward 合并进 master
- **M0 完成（2026-05-17）**：`server/` + `miniprogram/` 骨架进 mono-repo
...
1. **用户待操作（M2 验收）**：手机微信内置浏览器访问 cloudflared 临时域名下的 `/healthz`，确认返回 200 → M2 关闭
2. **M2 关闭后**：Wave 1 完成 → 可启动 M3（配对协议，task 待写）
3. **用户阻塞项**：小程序 AppID（M1 真机验收 + M3 配对协议必须，用户说"不得不用时再申请"）
4. **M3 前技术债**：`subpackages/crypto/index.js` 里 X25519 是手写 BigInt ladder，正式 E2EE 实现时需换 `@noble/curves/x25519`
5. **基础设施状态**：服务器（学长的，不用）· ICP 备案办理中（不阻塞 Phase 2）· 域名等备案下来 · 公司未注册（Phase 4 才需要）
