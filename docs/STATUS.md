...
**最后更新**：2026-05-13  ·  **当前版本**：**v3.0.0**（Unreleased 含 A2~A4 / B3~B6 技术债清理 + UI 一致性收尾）  ·  **阶段**：Alpha 本地 Demo · Phase 2 情感周报 v1 已发布 · task-20b/21 并行待开工
...
- **task-20b 任务卡入库**：合成剧本载体由 JSON 改为 Markdown 单源（人可读 + 可编辑 + 可重放 + 可从模板手写），driver/replay 仅换 IO 层；可与 task-21 并行 Wave 1
...
1. **task-20b / task-21 并行 Wave 1**：task-20b 改合成剧本载体为 MD（`docs/tasks/task-20b.md`，主改 `tools/synth/`），task-21 落地 PG + Docker Compose（`docs/tasks/task-21.md`，主改 `backend/infrastructure/` + 容器化）；核心文件集不重叠
2. **Phase 2 周报剩余技术债**：A1 + B7（搭车 task-21 处理）/ B1 暂搁置 / B2 由 task-20 合成语料替代 / C 系列详见 `docs/phase2_audit.md`
3. **另开 task 删除临时手动按钮**：`tab_us.py` 两处 `# TASK-9 临时按钮` 为定位标记，cron 验证稳定后清除
4. **收敛 `app_reports.md` 重复内容**：Report 模型 / reports_repo 同时存在于三份 L2，下次触碰 application 层时顺手清理
5. **deferred 到部署阶段（task-21 之后）**：`load_db()/save_db()` 整库改行级、`tick()` 独立调度器、累积 `_migrate_db` 清算
6. **task-22**：FastAPI 包装（前端 Streamlit → HTTP → FastAPI → application）
