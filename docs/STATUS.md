...
**最后更新**：2026-05-11  ·  **当前版本**：v2.2.0  ·  **阶段**：Alpha（本地 Demo） · 时间锁灵活化 task-4 完成

## 一句话项目摘要

OurPresent 是面向情侣的本地 Streamlit 私密记录空间，核心机制是「延时共享」时间锁（用户自选时长 + 立即/日历）和「冻结期销毁」分手协议。
...
1. **task-5：pending_unlock 流动性增强**（前置 task-4 已就绪，可启动）：追加内容、立即解锁、修改 unlock_at；UI 二次确认防止时间锁形同虚设
