...
**最后更新**：2026-05-11  ·  **当前版本**：v2.2.0  ·  **阶段**：Alpha（本地 Demo） · 本地阶段技术债已清

## 一句话项目摘要

OurPresent 是面向情侣的本地 Streamlit 私密记录空间，核心机制是「延时共享」时间锁（task-4 起改为用户自选时长）和「冻结期销毁」分手协议。
...
1. **task-4：自定义共享开放时间**（待启动）：替换固定 90 天，引入 `SessionRecord.unlock_at`，UI 提供预设档位 + 日历 + 「立即」
2. **task-5：pending_unlock 流动性增强**（依赖 task-4）：追加内容、立即解锁、修改 unlock_at；UI 二次确认防止时间锁形同虚设
