...
`docs/weekly_report.md` 已确定情感周报设计。本任务铺设数据底座，让后续 task-8 / task-9 能在稳定的字段与表上施工。**本任务不引入任何业务逻辑、不接 LLM、不改 UI。**
...
新增 dataclass，字段与设计稿 `docs/weekly_report.md` §3 一致：
...
- `docs/weekly_report.md` §3 / §8（Report schema、destruction 联动要求）
